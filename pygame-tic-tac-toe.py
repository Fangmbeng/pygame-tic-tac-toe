...
def draw_status():
    ...
    screen.fill((0, 0, 0), (0, 400, 475, 100))  # Correct space for status message
    text_rect = text.get_rect(center=(WIDTH / 2, HEIGHT + 50))  # Status bar placement
...
def check_win():
    ...
    for row in range(0, 3):
        if (grid[row][0] == grid[row][1] == grid[row][2]) and (grid[row][0] is not None):
            ...
            pg.draw.line(screen, (250, 0, 0), (0, (row + 1)*HEIGHT / 3 - HEIGHT / 6), (WIDTH, (row + 1)*HEIGHT / 3 - HEIGHT / 6), 7)  # Red line thickness increased
            break
    for col in range(0, 3):
        if (grid[0][col] == grid[1][col] == grid[2][col]) and (grid[0][col] is not None):
            current_winner = grid[0][col]
            pg.draw.line(screen, (250, 0, 0), ((col + 1) * WIDTH / 3 - WIDTH / 6, 0), ((col + 1) * WIDTH / 3 - WIDTH / 6, HEIGHT), 7)  # Red line thickness increased
            break
    if (grid[0][0] == grid[1][1] == grid[2][2]) and (grid[0][0] is not None):
        current_winner = grid[0][0]
        pg.draw.line(screen, (250, 0, 0), (50, 50), (350, 350), 7)  # Red line thickness increased
    if (grid[0][2] == grid[1][1] == grid[2][0]) and (grid[0][2] is not None):
        current_winner = grid[0][2]
        pg.draw.line(screen, (250, 0, 0), (350, 50), (50, 350), 7)  # Red line thickness increased
...
def reset_game():
    ...
    time.sleep(10)  # Game pauses for 10 seconds after a win or draw
    current_player = 'x'
    is_draw = False
